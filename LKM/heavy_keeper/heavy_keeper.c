...
//实现指数下降的概率
static inline int prob_action(u32 count){ //返回1 代表成功
	//随机采样
	u32 random_number = get_random_u32()% 100000 ;
	if(count < 11){
		if(random_number < 96336-5312*count) return 1;
	}else if(count <21){
		if(random_number < 69284-2463*count) return 1;
	}else if(count <31){
		if(random_number < 43533-1142*count) return 1;
	}else if(count <41){
		if(random_number < 25472-529*count) return 1;
	}else if(count <51){
		if(random_number <  14261-245*count) return 1;
	}else if(count <61){
		if(random_number <  7748-114*count) return 1;
	}else if(count <71){
		if(random_number <  4119-53*count) return 1;
	}else if(count <81){
		if(random_number <  2154-24*count) return 1;
	}else if(count <91){
		if(random_number <  1112-11*count) return 1;
	}else if(count <101){
		if(random_number <  568-5*count) return 1;
	}else {
		if(random_number < 1) return 1;
	}
...
static long heavy_keeper_update_elem(struct bpf_map *map, void *key, void *value, u64 flag) {
	...
	for (int i = 0; i < SKETCH_DEPTH; ++i) {
		u32 hash = fasthash32(key, SKETCH_KEY_SIZE, HASH_SEED_1) + i*fasthash32(key, SKETCH_KEY_SIZE, HASH_SEED_2);
		...
		if(flow_memcmp(tbl->flows + index,key) == 0){
			//是当前包对应流
			pr_debug("found matching key at %u array, %u bucket, count = %u ,src_ip=0x%08x", 
						i,hash % SKETCH_WIDTH,
						tbl->keys[index],
						tbl->flows[index].pkt.src_ip);
			tbl->keys[index]++;
		}else{
			pr_debug("not the matching key at %u array, %u bucket, count = %u ,src_ip=0x%08x ", 
						i,hash % SKETCH_WIDTH,
						tbl->keys[index],
						tbl->flows[index].pkt.src_ip);
			if(tbl->keys[index] > 0 && prob_action(tbl->keys[index]) == 1 ) tbl->keys[index]--;
			if(tbl->keys[index] == 0){
				tbl->flows[index]=*((sketch_key *)key);
				tbl->keys[index] = 1;
			}

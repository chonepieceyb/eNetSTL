...
static __always_inline int __cache_raw_sketch_primitive_update_get_elem(
	struct cache_raw_sketch_primitive_map *crskp, raw_skp_key_type *key)
{
	...
	/*cache full num == SIMD_CACHE_PKTS, use simd calculating hashes*/
...
#pragma GCC unroll 8
		for (i = 0; i < _HASH_BATCH_NUM; i++) {
			__m256i hh = _fasthash64_avx2(
				pkt, sizeof(struct pkt_5tuple), &simd_seeds[i]);
#pragma GCC unroll 8
			for (j = 0; j < _HASH_BATCH_SIZE; j++) {
				index = *((u32 *)&hh + j) & (COLUMNS - 1);
				count = ++*(arrays + row * COLUMNS + index);
				if (count < minimum)
					minimum = count;
				row += 1;
			}
		}
		cache->data[p].value = minimum;
		//cache->value[p] = minimum;
	}
	kernel_fpu_end();
...
#if PRINT_HASH_TIME
	pr_info("cach skp sidm update_get hash time used %llu\n", end - start);
#endif

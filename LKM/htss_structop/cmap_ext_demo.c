...
static struct bpf_map *map_ext_alloc(union bpf_attr *attr)
{
	...
	htss_map->ctx.__htss_memory = kmalloc(attr->key_size * NUM_BUCKETS * MEMBER_BUCKET_ENTRIES, GFP_ATOMIC);
...
{
	struct static_htss_map_structop *htss_map = container_of(map, struct static_htss_map_structop, map);
	struct htss_memory *__htss = htss_map->ctx.__htss_memory;

...
#define BUCKETLIST_NUM 2 * BUCKET_NUM
...
struct cffs_piq_map {
	...
	u32 cnt;
	DECLARE_FLEX_ARRAY(struct list_head __percpu *, bkts)
...
static __always_inline void __init_bkt_list(int cpu, struct cffs_piq_map *cmap, u32 max_entries) 
{
	u32 i;
	for (i = 0; i < max_entries; i++) {
		struct list_head *head = per_cpu_ptr(cmap->bkts[i], cpu);
		INIT_LIST_HEAD(head);
	}
}

static __always_inline void
__free_bkt_list_entries(int cpu, struct cffs_piq_map *cmap, u32 max_entries)
{
	u32 i;
	/* free the list entry*/
	...
	for (i = 0; i < max_entries; i++) {
		...
		list_for_each_entry_safe(entry, n, head, node) {
			...
			list_del(&entry->node);
			bpf_mem_cache_free(&cmap->ma, entry);
		}
	}
	migrate_enable();
}

static __always_inline void __free_bkt_list(struct cffs_piq_map *cmap,
					    u32 max_entries)
{
	u32 i;
	for (i = 0; i < max_entries; i++) {
...
static void cffs_free_bkts(struct cffs_piq_map *cmap)
{
	...
	for_each_possible_cpu(cpu) {
		__free_bkt_list_entries(cpu, cmap, BUCKETLIST_NUM);
	}
	__free_bkt_list(cmap, BUCKETLIST_NUM);
}

/*
*@return 0 means success
*/
static int cffs_alloc_bkt_lists(struct cffs_piq_map *cmap, u32 max_entries)
{
	u32 i;

	for (i = 0; i < max_entries; i++) {
		cmap->bkts[i] = __alloc_percpu_gfp(sizeof(struct list_head),
						   __alignof__(u64),
	...
	return 0;
err_free:;
	__free_bkt_list(cmap, max_entries);
	return -ENOMEM;
}
...
static struct bpf_map *cffs_piq_alloc(union bpf_attr *attr)
{
	...
	u32 __value_size = sizeof(struct bktlist_entry) + attr->value_size;
	if (bpf_mem_alloc_init(&cmap->ma, __value_size, false)) {
	...
	if (cffs_alloc_bkt_lists(cmap, BUCKETLIST_NUM)) {
		res_ptr = ERR_PTR(-ENOMEM);
		goto free_cffs;
	}
	/*init bkt list*/
	for_each_possible_cpu(cpu) {
		__init_bkt_list(cpu, cmap, BUCKETLIST_NUM);
	}
				 ...
				 u64 flags)
{
	...
	head = this_cpu_ptr(cmap->bkts[__bkt_idx]);

	// alloc a new bucketlist entry
	new_entry = bpf_mem_cache_alloc(&cmap->ma);
	if (new_entry == NULL) {
	...
	// set the entry data and add it to the bucket list tail
	memcpy(new_entry->data, __value, sizeof(struct __cffs_value_type));
...
static long cffs_piq_pop_elem(struct bpf_map *map, void *value)
{
	...
	head = this_cpu_ptr(cmap->bkts[bkt_idx]);
	struct bktlist_entry *first_entry = list_first_entry(head, struct bktlist_entry, node);
	memcpy(value, first_entry->data, sizeof(struct __cffs_value_type)); /*copy to ebpf*/
	list_del(&first_entry->node);
	bpf_mem_cache_free(&cmap->ma, first_entry);
	cmap->cnt -= 1;
...
static u64 cffs_piq_mem_usage(const struct bpf_map *map)
{
	...
	used += (sizeof(struct bktlist_entry) + map->value_size) * (cmap->cnt) *
		num_possible_cpus();
	return used;
}

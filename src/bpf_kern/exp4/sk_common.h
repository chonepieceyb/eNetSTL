/*
 *copy from https://github.com/QMUL-EECS-Networks-Systems/ebpf-sketches
 */
...
{
	x = ~x;
	const uint32_t numIntBits =
		sizeof(uint32_t) * 8; //compile time constant
	//do the smearing
	x |= x >> 1;
	x |= x >> 2;
	x |= x >> 4;
...
static int __always_inline median(int *vect, int len)
{
	...
	if (len % 2 == 0) {
		...
		return average_without_overflow(vect[len / 2],
						vect[len / 2 - 1]);
	} else {
...
#if RANDOM_PORT_OVERRIDE
#define PORT(x) (bpf_ktime_get_ns() & ((1 << RANDOM_PORT_MASK_BITS) - 1))
#else
#define PORT(x) (x)
#endif

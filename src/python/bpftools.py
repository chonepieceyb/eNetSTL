...
if __name__ == '__main__':
    from common import *
    import os 
    KPROBE_DEMO_OBJ_PATH = os.path.join(BPF_KERN_OBJ_DIR, 'kprobe_demo_kern.c.o')
    PROG_NAME = '__sys_connect'
    with BPFObject(KPROBE_DEMO_OBJ_PATH) as bpf_obj:
        bpf_obj.load()
        bpf_prog = bpf_obj.get_prog("kprobe_demo")
        bpf_map = BPFMap(bpf_obj.get_map("fd_array"), ct.c_int, ct.c_int)
        with BPFLink(bpf_program__attach_kprobe, bpf_prog, False, PROG_NAME) as link: 
            link.disconnect()
            try:
                while True:
                    pass
                ...
                pass
            print("fd : %d"%bpf_map.lookup(ct.c_int(0)).value)
